...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'END:VCARD'
// JANGAN LUPA SUBSCRIBE CHANNEL MRKING
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SUBSCRIBE YT MR KING CHANNEL'))
})
	...
	baby.on('connecting', () => {
		start('2', 'MrKing Connecting...')
	})
	baby.on('open', () => {
		success('2', 'MrKing Connected')
	})
...
// ANTI LINK GRUP
		        ...
		        Link Group Terdeteksi maaf ${sender.split("@")[0]} anda akan di kick dari group 3detik lagi`)
		        setTimeout( () => {
...
*/
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 tes`)
				ramlan = body.slice(8)
				reply('「❗」WAIT GAN GUA NULIS DULU YAA')
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo MrKing & Gans`)
				reply(ind.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub Farhan & Hub`)
				reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}gemboktext 11 01 2021 & Putra dan Putri`)
					reply(ind.wait())
				...
				if (!isPrem) return reply(ind.premium())
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}glitchtext MrKing & Gans`)
					reply(ind.wait())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts id Halo Gan', text, {quoted: mek})
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata MrKing`
...
case 'neontext':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}neontext MrKing`)
neon = body.slice(10)
reply('「❗」WAIT GANS')
...
*/		 
				case 'welcome':
                  if (isBanned) return reply(ind.baned())				
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*FITUR WELCOME SUDAH AKTIF GAN*')
						welkom.push(from)
...
case 'blackpink':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink MrKing`)
pink = body.slice(11)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner MrKing`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena MrKing baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
case 'cloudtext':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}cloudtext MrKing`)
cloud = body.slice(11)
